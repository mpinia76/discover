...
if(mysqli_num_rows($result) != 0){
	...
	if ($rsCompra['estado']==1) {
        if (empty($rsCompra['plan_id'])){
            $sql = "SELECT * FROM cuenta_a_pagar WHERE operacion_id = ".$rsCompra['id']." AND operacion_tipo = 'compra' AND plan_id is not null";
            $result = mysqli_query($conn,$sql);
            if(mysqli_num_rows($result) == 0){
                $ok=1;
                ...
                echo "La compra se encuentra incluida en un plan de pagos en cuentas a pagar";
            }


        }else{
            echo "La compra se encuentra incluida en un plan de pagos";
        }
